...
CLIENT_TARGET_BASENAME :=client
CLIENT_TARGET :=$(BIN_DIR)/$(CLIENT_TARGET_BASENAME).exe
...
BOOST_LIBS    :=-lboost_filesystem-mgw13-mt-x64-1_83 \
                ...
                -lboost_serialization-mgw13-mt-x64-1_83 \
...
# for windows build
WINDOWS_LIB :=-lole32 -luuid -lshell32 -lws2_32
...
CLIENT_CPP         := $(filter-out %main.cpp,$(SOURCES_CPP))
CLIENT_CPP_OBJECTS := $(addprefix $(OBJECTS_DIR)/,$(notdir $(CLIENT_CPP:.cpp=.o)))

MAIN_CPP         := $(filter-out %client.cpp,$(SOURCES_CPP))
MAIN_CPP_OBJECTS := $(addprefix $(OBJECTS_DIR)/,$(notdir $(MAIN_CPP:.cpp=.o)))

.PHONY: all info clean run client

all: $(MAIN_TARGET) $(LUA_MOD) $(LUA_PACKAGE) client
client: $(CLIENT_TARGET)

$(MAIN_TARGET): $(MAIN_CPP_OBJECTS)
	$(CXX) -o $@ $^ $(CXXLDFLAGS)

$(CLIENT_TARGET): $(CLIENT_CPP_OBJECTS)
	$(CXX) -o $@ $^ $(CXXLDFLAGS)
